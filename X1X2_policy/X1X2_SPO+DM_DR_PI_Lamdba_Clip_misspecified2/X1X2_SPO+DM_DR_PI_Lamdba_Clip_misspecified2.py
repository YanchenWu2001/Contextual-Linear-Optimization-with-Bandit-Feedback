from utility_functions_sgd_linear import * 
import numpy as np
...
def Double_robust(X_train_False,X_train, Y_transpose_Z_observed_train, Y_transpose_Z_expected_train,Z_train,Z_train_idxs_1,
    ...
    Tree_Linear_Wrong_Double_Robust_2=np.zeros((d,m))
    Tree_Clip_Linear_Wrong_Double_Robust_2=np.zeros((d,m))
    Tree_Lambda_Linear_Wrong_Double_Robust_2=np.zeros((d,m))
    ...
    for k in range(m):
        ...
        Tree_Linear_Wrong_Double_Robust_2[:,k]=Linear_Wrong_model_estimate_2[:,k]+np.dot(Tree_estimate_Sigma_inverse,Z_2[:,k])*(np.array(Y_2.iloc[k,:])-wrong_linear_model_predicts_2[k,:])
        Tree_Clip_Linear_Wrong_Double_Robust_2[:,k]=Linear_Wrong_model_estimate_2[:,k]+np.dot(Modeified_Tree_Sigma_inverse,Z_2[:,k])*(np.array(Y_2.iloc[k,:])-wrong_linear_model_predicts_2[k,:])
        Tree_Lambda_Linear_Wrong_Double_Robust_2[:,k]=Linear_Wrong_model_estimate_2[:,k]+np.dot(Tree_Sigma_matrix_adjustment_inverse,Z_2[:,k])*(np.array(Y_2.iloc[k,:])-wrong_linear_model_predicts_2[k,:])

        Tree_Vanilla_Double_robust_predict_2[:,k]=model_estimate_2[:,k]+np.dot(Tree_estimate_Sigma_inverse,Z_2[:,k])*(np.array(Y_2.iloc[k,:])-predicts_2[k,:])
        Tree_Clip_Double_robust_predict_2[:,k]=model_estimate_2[:,k]+np.dot(Modeified_Tree_Sigma_inverse,Z_2[:,k])*(np.array(Y_2.iloc[k,:])-predicts_2[k,:])
        Tree_Lambda_Double_robust_predict_2_tree[:,k]=model_estimate_2[:,k]+np.dot(Tree_Sigma_matrix_adjustment_inverse,Z_2[:,k])*(np.array(Y_2.iloc[k,:])-predicts_2[k,:])
    ...
    for i in range(m):
         if X[1,i]>0 and X[2,i]>0:
             a=np.array(Y_2.iloc[i,:])
             b=wrong_linear_model_predicts_2[i,:]
             c=a-b
             Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_1_sample_inverse,Z_2[:,i])*c
             Clip_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Modeified_sample_Sigma_matrix_1_inverse,Z_2[:,i])*c
             Lambda_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_1_adjustment_sample_lambda_inverse,Z_2[:,i])*c
             
         if X[1,i]>0 and X[2,i]<=0:
             a=np.array(Y_2.iloc[i,:])
             b=wrong_linear_model_predicts_2[i,:]
             c=a-b
             Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_2_sample_inverse,Z_2[:,i])*c
             Clip_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Modeified_sample_Sigma_matrix_2_inverse,Z_2[:,i])*c
             Lambda_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_2_adjustment_sample_lambda_inverse,Z_2[:,i])*c

         if X[1,i]<=0 and X[2,i]>0:
             a=np.array(Y_2.iloc[i,:])
             b=wrong_linear_model_predicts_2[i,:]
             c=a-b
             Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_3_sample_inverse,Z_2[:,i])*c
             Clip_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Modeified_sample_Sigma_matrix_3_inverse,Z_2[:,i])*c
             Lambda_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_3_adjustment_sample_lambda_inverse,Z_2[:,i])*c

         if X[1,i]<=0 and X[2,i]<=0:
             a=np.array(Y_2.iloc[i,:])
             b=wrong_linear_model_predicts_2[i,:]
             c=a-b
             Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_4_sample_inverse,Z_2[:,i])*c
             Clip_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Modeified_sample_Sigma_matrix_4_inverse,Z_2[:,i])*c
             Lambda_Linear_Wrong_Double_Robust_2_sample[:,i]=Linear_Wrong_model_estimate_2[:,i]+np.dot(Sigma_matrix_4_adjustment_sample_lambda_inverse,Z_2[:,i])*c
             
    return(model_estimate_2,Vanilla_Double_robust_predict_2_sample,Clip_Double_robust_predict_2_sample,Lambda_Double_robust_predict_2_sample,Linear_Wrong_model_estimate_2,
           Linear_Wrong_Double_Robust_2_sample,Clip_Linear_Wrong_Double_Robust_2_sample,Lambda_Linear_Wrong_Double_Robust_2_sample,Tree_Linear_Wrong_Double_Robust_2,
           Tree_Clip_Linear_Wrong_Double_Robust_2,Tree_Lambda_Linear_Wrong_Double_Robust_2,Tree_Vanilla_Double_robust_predict_2,Tree_Clip_Double_robust_predict_2,Tree_Lambda_Double_robust_predict_2_tree)
...
regret_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
validation_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
time_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
loss_all_Linear_Wrong_Double_Robust_Tree_Sigma = []

Clip_regret_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
Clip_validation_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
Clip_time_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
Clip_loss_all_Linear_Wrong_Double_Robust_Tree_Sigma = []

Lambda_regret_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
Lambda_validation_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
Lambda_time_all_Linear_Wrong_Double_Robust_Tree_Sigma = []
Lambda_loss_all_Linear_Wrong_Double_Robust_Tree_Sigma = []

regret_all_Sample_Double_Robust_tree = []
validation_all_Sample_Double_Robust_tree = []
time_all_Sample_Double_Robust_tree = []
loss_all_Sample_Double_Robust_tree = []

Clip_regret_all_Sample_Double_Robust_tree = []
Clip_validation_all_Sample_Double_Robust_tree = []
Clip_time_all_Sample_Double_Robust_tree = []
Clip_loss_all_Sample_Double_Robust_tree = []

Lambda_regret_all_Sample_Double_Robust_tree = []
Lambda_validation_all_Sample_Double_Robust_tree = []
Lambda_time_all_Sample_Double_Robust_tree = []
Lambda_loss_all_Sample_Double_Robust_tree = []
...
for i in range(len(n_train_seq)):
    ...
    res_temp = Parallel(n_jobs = n_jobs, verbose = 3)(delayed(replication_no_kernel)(
                            A_mat, b_vec, B_true, 
                            data_train[run][0][:, :n_train], data_train[run][1][:, :n_train], data_train[run][12][:, :n_train], data_train[run][3][:, :n_train],
                            data_holdout[run][0][:, :n_holdout], data_holdout[run][1][:, :n_holdout], data_holdout[run][12][:, :n_holdout], data_holdout[run][3][:, :n_holdout],
                            data_test[0], data_test[1], data_test[2], data_test[3],
                            # numiter = numiter, batchsize = batchsize, long_factor = long_factor,
                            #   loss_stop = loss_stop, tol = tol, stop = stop,
                            gammas = gammas, lambdas = lambdas, lambda_max = None, lambda_min_ratio = None, num_lambda = None,
    ...
    with open(output, 'a') as f:
        print("Linear Wrong Double Robust Tree Sigma")
        print("Linear Wrong Double Robust Tree Sigma",file = f)
        print("total time: ", time.time() - time1, file = f)
        print("seperate time", file = f)
        print(time_temp.mean(), file = f)
        print("-------", file = f)
    ...
    regret_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(regret_temp)
    validation_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(validation_temp)
    time_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(time_temp)
    loss_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(loss_temp)
    
    pd.concat(regret_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("regret_all_Linear_Wrong_Double_Robust_Tree_Sigma.csv", index = False)
    pd.concat(validation_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("validation_no_kernel_Linear_Wrong_Double_Robust_Samole_Sigma.csv", index = False)
    pd.concat(time_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("time_no_kerne_Linear_Wrong_Double_Robust_Tree_Sigma.csv", index = False)
    pickle.dump(loss_all_Linear_Wrong_Double_Robust_Tree_Sigma, open("loss_no_kernel_Linear_Wrong_Double_Robust_Tree_Sigma.pkl", "wb")) 
    ...
    res_temp = Parallel(n_jobs = n_jobs, verbose = 3)(delayed(replication_no_kernel)(
                            A_mat, b_vec, B_true, 
                            data_train[run][0][:, :n_train], data_train[run][1][:, :n_train], data_train[run][13][:, :n_train], data_train[run][3][:, :n_train],
                            data_holdout[run][0][:, :n_holdout], data_holdout[run][1][:, :n_holdout], data_holdout[run][13][:, :n_holdout], data_holdout[run][3][:, :n_holdout],
                            data_test[0], data_test[1], data_test[2], data_test[3],
                            # numiter = numiter, batchsize = batchsize, long_factor = long_factor,
                            #   loss_stop = loss_stop, tol = tol, stop = stop,
                            gammas = gammas, lambdas = lambdas, lambda_max = None, lambda_min_ratio = None, num_lambda = None,
    ...
    with open(output, 'a') as f:
        print("Linear Wrong Double Robust Tree Sigma Clip")
        print("Linear Wrong Double Robust Tree Sigma Clip",file = f)
        print("total time: ", time.time() - time1, file = f)
        print("seperate time", file = f)
        print(time_temp.mean(), file = f)
        print("-------", file = f)
    ...
    Clip_regret_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(regret_temp)
    Clip_validation_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(validation_temp)
    Clip_time_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(time_temp)
    Clip_loss_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(loss_temp)
    
    pd.concat(Clip_regret_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("regret_no_kernel_Clip_Linear_Wrong_Double_Robust_Tree_Sigma.csv", index = False)
    pd.concat(Clip_validation_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("validation_no_kernel_Clip_Linear_Wrong_Double_Robust_Samole_Sigma.csv", index = False)
    pd.concat(Clip_time_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("time_no_kerne_Clip_Linear_Wrong_Double_Robust_Tree_Sigma.csv", index = False)
    pickle.dump(Clip_loss_all_Linear_Wrong_Double_Robust_Tree_Sigma, open("loss_no_kernel_Clip_Linear_Wrong_Double_Robust_Tree_Sigma.pkl", "wb")) 
    ...
    res_temp = Parallel(n_jobs = n_jobs, verbose = 3)(delayed(replication_no_kernel)(
                            A_mat, b_vec, B_true, 
                            data_train[run][0][:, :n_train], data_train[run][1][:, :n_train], data_train[run][14][:, :n_train], data_train[run][3][:, :n_train],
                            data_holdout[run][0][:, :n_holdout], data_holdout[run][1][:, :n_holdout], data_holdout[run][14][:, :n_holdout], data_holdout[run][3][:, :n_holdout],
                            data_test[0], data_test[1], data_test[2], data_test[3],
                            # numiter = numiter, batchsize = batchsize, long_factor = long_factor,
                            #   loss_stop = loss_stop, tol = tol, stop = stop,
                            gammas = gammas, lambdas = lambdas, lambda_max = None, lambda_min_ratio = None, num_lambda = None,
    ...
    with open(output, 'a') as f:
        print("Linear Wrong Double Robust Tree Sigma Lambda")
        print("Linear Wrong Double Robust Tree Sigma Lambda",file = f)
        print("total time: ", time.time() - time1, file = f)
        print("seperate time", file = f)
        print(time_temp.mean(), file = f)
        print("-------", file = f)
    ...
    Lambda_regret_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(regret_temp)
    Lambda_validation_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(validation_temp)
    Lambda_time_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(time_temp)
    Lambda_loss_all_Linear_Wrong_Double_Robust_Tree_Sigma.append(loss_temp)
    
    pd.concat(Lambda_regret_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("regret_no_kernel_Lambda_Linear_Wrong_Double_Robust_Tree_Sigma.csv", index = False)
    pd.concat(Lambda_validation_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("validation_no_kernel_Lambda_Linear_Wrong_Double_Robust_Samole_Sigma.csv", index = False)
    pd.concat(Lambda_time_all_Linear_Wrong_Double_Robust_Tree_Sigma).to_csv("time_no_kerne_Lambda_Linear_Wrong_Double_Robust_Tree_Sigma.csv", index = False)
    pickle.dump(Lambda_loss_all_Linear_Wrong_Double_Robust_Tree_Sigma, open("loss_no_kernel_Lambda_Linear_Wrong_Double_Robust_Tree_Sigma.pkl", "wb")) 
    ...
    res_temp = Parallel(n_jobs = n_jobs, verbose = 3)(delayed(replication_no_kernel)(
                            A_mat, b_vec, B_true, 
                            data_train[run][0][:, :n_train], data_train[run][1][:, :n_train], data_train[run][15][:, :n_train], data_train[run][3][:, :n_train],
                            data_holdout[run][0][:, :n_holdout], data_holdout[run][1][:, :n_holdout], data_holdout[run][15][:, :n_holdout], data_holdout[run][3][:, :n_holdout],
                            data_test[0], data_test[1], data_test[2], data_test[3],
                            # numiter = numiter, batchsize = batchsize, long_factor = long_factor,
                            #   loss_stop = loss_stop, tol = tol, stop = stop,
                            gammas = gammas, lambdas = lambdas, lambda_max = None, lambda_min_ratio = None, num_lambda = None,
    ...
    with open(output, 'a') as f:
        print("Double Robust With Sample Sigma Matrix Tree")
        print("Double Robust With Sample Sigma Matrix Tree", file = f)
        print("total time: ", time.time() - time1, file = f)
        print("seperate time", file = f)
        print(time_temp.mean(), file = f)
        print("-------", file = f)
    ...
    regret_all_Sample_Double_Robust_tree.append(regret_temp)
    validation_all_Sample_Double_Robust_tree.append(validation_temp)
    time_all_Sample_Double_Robust_tree.append(time_temp)
    loss_all_Sample_Double_Robust_tree.append(loss_temp)
    
    pd.concat(regret_all_Sample_Double_Robust_tree).to_csv("regret_no_kernel_Sample_Double_Robust_tree.csv", index = False)
    pd.concat(validation_all_Sample_Double_Robust_tree).to_csv("validation_no_kernel_Sample_Double_Robust_tree.csv", index = False)
    pd.concat(time_all_Sample_Double_Robust_tree).to_csv("time_no_kernel_Sample_Double_Robust_tree.csv", index = False)
    pickle.dump(loss_all_Sample_Double_Robust_tree, open("loss_no_kernel_Sample_Double_Robust_tree.pkl", "wb"))      
    ...
    with open(output, 'a') as f:
        print("Double Robust With Sample Sigma Matrix Clip Tree")
        print("Double Robust With Sample Sigma Matrix Clip Tree", file = f)
        print("total time: ", time.time() - time1, file = f)
        print("seperate time", file = f)
        print(time_temp.mean(), file = f)
        print("-------", file = f)
    ...
    Clip_regret_all_Sample_Double_Robust_tree.append(regret_temp)
    Clip_validation_all_Sample_Double_Robust_tree.append(validation_temp)
    Clip_time_all_Sample_Double_Robust_tree.append(time_temp)
    Clip_loss_all_Sample_Double_Robust_tree.append(loss_temp)
    
    pd.concat(Clip_regret_all_Sample_Double_Robust_tree).to_csv("regret_no_kernel_Clip_Sample_Double_Robust_tree.csv", index = False)
    pd.concat(Clip_validation_all_Sample_Double_Robust_tree).to_csv("validation_no_kernel_Clip_Sample_Double_Robust_tree.csv", index = False)
    pd.concat(Clip_time_all_Sample_Double_Robust_tree).to_csv("time_no_kernel_Clip_Sample_Double_Robust_tree.csv", index = False)
    pickle.dump(Clip_loss_all_Sample_Double_Robust_tree, open("loss_no_kernel_Clip_Sample_Double_Robust_tree.pkl", "wb"))
    ...
    res_temp = Parallel(n_jobs = n_jobs, verbose = 3)(delayed(replication_no_kernel)(
                            A_mat, b_vec, B_true, 
                            data_train[run][0][:, :n_train], data_train[run][1][:, :n_train], data_train[run][17][:, :n_train], data_train[run][3][:, :n_train],
                            data_holdout[run][0][:, :n_holdout], data_holdout[run][1][:, :n_holdout], data_holdout[run][17][:, :n_holdout], data_holdout[run][3][:, :n_holdout],
                            data_test[0], data_test[1], data_test[2], data_test[3],
                            # numiter = numiter, batchsize = batchsize, long_factor = long_factor,
                            #   loss_stop = loss_stop, tol = tol, stop = stop,
                            gammas = gammas, lambdas = lambdas, lambda_max = None, lambda_min_ratio = None, num_lambda = None,
    ...
    with open(output, 'a') as f:
        print("Double Robust With Sample Sigma Matrix Lambda Tree")
        print("Double Robust With Sample Sigma Matrix Lambda Tree", file = f)
        print("total time: ", time.time() - time1, file = f)
        print("seperate time", file = f)
        print(time_temp.mean(), file = f)
        print("-------", file = f)
    ...
    Lambda_regret_all_Sample_Double_Robust_tree.append(regret_temp)
    Lambda_validation_all_Sample_Double_Robust_tree.append(validation_temp)
    Lambda_time_all_Sample_Double_Robust_tree.append(time_temp)
    Lambda_loss_all_Sample_Double_Robust_tree.append(loss_temp)
    
    pd.concat(Lambda_regret_all_Sample_Double_Robust_tree).to_csv("regret_no_kernel_Lambda_Sample_Double_Robust_tree.csv", index = False)
    pd.concat(Lambda_validation_all_Sample_Double_Robust_tree).to_csv("validation_no_kernel_Lambda_Sample_Double_Robust_tree.csv", index = False)
    pd.concat(Lambda_time_all_Sample_Double_Robust_tree).to_csv("time_no_kernel_Lambda_Sample_Double_Robust_tree.csv", index = False)
    pickle.dump(Lambda_loss_all_Sample_Double_Robust_tree, open("loss_no_kernel_Lambda_Sample_Double_Robust_tree.pkl", "wb"))