...
regret_all_Wrong_IPW_tree_DR = []
validation_all_Wrong_IPW_tree_DR = []
time_all_Wrong_IPW_tree_DR = []
loss_all_Wrong_IPW_tree_DR = []

regret_all_IPW_tree_DR= []
validation_all_IPW_tree_DR = []
time_all_IPW_tree_DR = []
loss_all_IPW_tree_DR = []
for i in range(len(n_train_seq)):
    ...
    for k in range(runs):
        ...
        for j in range(n_train_true):
             if data_train[k][0][1,j]>0 and data_train[k][0][2,j]>0:
               false_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])/IPW_score_1[int(data_train[k][8][0,j])]
               true_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])/IPW_score_1[int(data_train[k][8][0,j])]
             if data_train[k][0][1,j]>0 and data_train[k][0][2,j]<=0:
               false_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])/IPW_score_2[int(data_train[k][8][0,j])]
               true_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])/IPW_score_2[int(data_train[k][8][0,j])]
             if data_train[k][0][1,j]<=0 and data_train[k][0][2,j]>0:
               false_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])/IPW_score_3[int(data_train[k][8][0,j])]
               true_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])/IPW_score_3[int(data_train[k][8][0,j])]
             if data_train[k][0][1,j]<=0 and data_train[k][0][2,j]<=0:
               false_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])/IPW_score_4[int(data_train[k][8][0,j])]
               true_data_train_cost_hat[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])/IPW_score_4[int(data_train[k][8][0,j])]
    ...
    for k in range(runs):
        ...
        for j in range(n_train_true):
            if data_train[k][0][1,j]>0 and data_train[k][0][2,j]>0 and (data_train[k][7][0,j]<25 or data_train[k][7][0,j]>=50): 
              false_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=(75/2)*(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])
              true_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=(75/2)*(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])
            if data_train[k][0][1,j]>0 and data_train[k][0][2,j]>0 and (25<=data_train[k][7][0,j]<50): 
              false_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=75*(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])
              true_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=75*(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])
            ...
            if data_train[k][0][1,j]<=0 and data_train[k][0][2,j]>0 and (data_train[k][7][0,j]<25 or data_train[k][7][0,j]>=50): 
              false_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=(100/3)*(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])
              true_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=(100/3)*(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])
            if data_train[k][0][1,j]<=0 and data_train[k][0][2,j]>0 and (25<=data_train[k][7][0,j]<50): 
              false_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=100*(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])
              true_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=100*(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])
            if data_train[k][0][1,j]<=0 and data_train[k][0][2,j]<=0 and data_train[k][7][0,j]>=25: 
              false_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=(100/3)*(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])
              true_data_train_cost_ideal_hat[int(data_train[k][8][0,j]),j]=(100/3)*(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])
            if data_train[k][0][1,j]<=0 and data_train[k][0][2,j]<=0 and data_train[k][7][0,j]<25: 
    ...
    for k in range(runs):
        ...
        for j in range(n_holdout_true):
             if data_holdout[k][0][1,j]>0 and data_holdout[k][0][2,j]>0:
               false_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])/IPW_score_1[int(data_holdout[k][8][0,j])]
               true_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])/IPW_score_1[int(data_holdout[k][8][0,j])]
             if data_holdout[k][0][1,j]>0 and data_holdout[k][0][2,j]<=0:
               false_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])/IPW_score_2[int(data_holdout[k][8][0,j])]
               true_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])/IPW_score_2[int(data_holdout[k][8][0,j])]
             if data_holdout[k][0][1,j]<=0 and data_holdout[k][0][2,j]>0:
               false_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])/IPW_score_3[int(data_holdout[k][8][0,j])]
               true_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])/IPW_score_3[int(data_holdout[k][8][0,j])]
             if data_holdout[k][0][1,j]<=0 and data_holdout[k][0][2,j]<=0:
               false_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])/IPW_score_4[int(data_holdout[k][8][0,j])]
               true_data_holdout_cost_hat[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])/IPW_score_4[int(data_holdout[k][8][0,j])]
    ...
    for k in range(runs):
        ...
        for j in range(n_holdout_true):
            if data_holdout[k][0][1,j]>0 and data_holdout[k][0][2,j]>0 and (data_holdout[k][7][0,j]<25 or data_holdout[k][7][0,j]>=50): 
              false_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=(75/2)*(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])
              true_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=(75/2)*(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])
            if data_holdout[k][0][1,j]>0 and data_holdout[k][0][2,j]>0 and (25<=data_holdout[k][7][0,j]<50): 
              false_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=75*(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])
              true_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=75*(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])
            ...
            if data_holdout[k][0][1,j]<=0 and data_holdout[k][0][2,j]>0 and (data_holdout[k][7][0,j]<25 or data_holdout[k][7][0,j]>=50): 
              false_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=(100/3)*(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])
              true_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=(100/3)*(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])
            if data_holdout[k][0][1,j]<=0 and data_holdout[k][0][2,j]>0 and (25<=data_holdout[k][7][0,j]<50): 
              false_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=100*(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])
              true_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=100*(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])
            if data_holdout[k][0][1,j]<=0 and data_holdout[k][0][2,j]<=0 and data_holdout[k][7][0,j]>=25: 
              false_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=(100/3)*(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])
              true_data_holdout_cost_ideal_hat[int(data_holdout[k][8][0,j]),j]=(100/3)*(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])
            if data_holdout[k][0][1,j]<=0 and data_holdout[k][0][2,j]<=0 and data_holdout[k][7][0,j]<25: 
    ...
    for k in range(runs):        
        train_DR_array_false_Sample_IPW=data_train[k][11]+data_train[k][9]
        train_DR_array_false_Ideal_IPW=data_train[k][13]+data_train[k][9]
        train_DR_array_true_Sample_IPW=data_train[k][12]+data_train[k][10]
        train_DR_array_true_Ideal_IPW=data_train[k][14]+data_train[k][10]
        ...
        holdout_DR_array_false_Sample_IPW=data_holdout[k][11]+data_holdout[k][9]
        holdout_DR_array_false_Ideal_IPW=data_holdout[k][13]+data_holdout[k][9]
        holdout_DR_array_true_Sample_IPW=data_holdout[k][12]+data_holdout[k][10]
        holdout_DR_array_true_Ideal_IPW=data_holdout[k][14]+data_holdout[k][10]
    ...
    for k in range(runs):
        false_data_train_cost_hat_tree=np.zeros((70,n_train_true))
        true_data_train_cost_hat_tree=np.zeros((70,n_train_true))
        ...
        for j in range(n_train_true):
            for i in range(len(unique)):
                if data_train[k][8][0,j]==unique[i]:
                    false_data_train_cost_hat_tree[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][9][int(data_train[k][8][0,j]),j])/a_2[i,j]
                    true_data_train_cost_hat_tree[int(data_train[k][8][0,j]),j]=(data_train[k][4][0,j]-data_train[k][10][int(data_train[k][8][0,j]),j])/a_2[i,j]

        data_train[k].append(false_data_train_cost_hat_tree)
        data_train[k].append(true_data_train_cost_hat_tree)
    

    for k in range(runs):
        false_data_holdout_cost_hat_tree=np.zeros((70,n_holdout_true))
        true_data_holdout_cost_hat_tree=np.zeros((70,n_holdout_true))
        clf = tree.DecisionTreeClassifier(max_depth=3,random_state=18)
        ...
        for j in range(n_holdout_true):
            for i in range(len(unique)):
                if data_holdout[k][8][0,j]==unique[i]:
                    false_data_holdout_cost_hat_tree[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][9][int(data_holdout[k][8][0,j]),j])/a_2[i,j]
                    true_data_holdout_cost_hat_tree[int(data_holdout[k][8][0,j]),j]=(data_holdout[k][4][0,j]-data_holdout[k][10][int(data_holdout[k][8][0,j]),j])/a_2[i,j]
        data_holdout[k].append(false_data_holdout_cost_hat_tree)
        data_holdout[k].append(true_data_holdout_cost_hat_tree)
    
    for k in range(runs):        
        train_DR_array_false_tree_IPW=data_train[k][19]+data_train[k][9]
        train_DR_array_true_tree_IPW=data_train[k][20]+data_train[k][10]
        data_train[k].append(train_DR_array_false_tree_IPW)
        data_train[k].append(train_DR_array_true_tree_IPW)

        holdout_DR_array_false_tree_IPW=data_holdout[k][19]+data_holdout[k][9]
        holdout_DR_array_true_tree_IPW=data_holdout[k][20]+data_holdout[k][10]
        data_holdout[k].append(holdout_DR_array_false_tree_IPW)
        data_holdout[k].append(holdout_DR_array_true_tree_IPW)
    ...
    res_temp = Parallel(n_jobs = n_jobs, verbose = 3)(delayed(replication_no_kernel)(
                            ...
                            data_train[run][0][:, :n_train_true], data_train[run][1][:, :n_train_true], data_train[run][21][:, :n_train_true], data_train[run][21][:, :n_train_true],
                            data_holdout[run][0][:, :n_holdout_true], data_holdout[run][1][:, :n_holdout_true], data_holdout[run][21][:, :n_holdout_true], data_holdout[run][21][:, :n_holdout_true],
    ...
    with open(output, 'a') as f:
        print("Wrong tree CDR")
        print("Wrong tree CDR", file = f)
    ...
    regret_all_Wrong_IPW_tree_DR.append(regret_temp)
    validation_all_Wrong_IPW_tree_DR.append(validation_temp)
    time_all_Wrong_IPW_tree_DR.append(time_temp)
    loss_all_Wrong_IPW_tree_DR.append(loss_temp)
    
    pd.concat(regret_all_Wrong_IPW_tree_DR).to_csv("regret_no_kernel_Wrong_tree_CDR.csv", index = False)
    pd.concat(validation_all_Wrong_IPW_tree_DR).to_csv("validation_no_kernel_Wrong_tree_CDR.csv", index = False)
    pd.concat(time_all_Wrong_IPW_tree_DR).to_csv("time_no_kernel_Wrong_tree_CDR.csv", index = False)
    pickle.dump(loss_all_Wrong_IPW_tree_DR, open("loss_no_kernel_Wrong_tree_CDR.pkl", "wb"))
    ...
    res_temp = Parallel(n_jobs = n_jobs, verbose = 3)(delayed(replication_no_kernel)(
                            A_mat_new, b_vec_new, B_true, 
                            data_train[run][0][:, :n_train_true], data_train[run][1][:, :n_train_true], data_train[run][22][:, :n_train_true], data_train[run][22][:, :n_train_true],
                            data_holdout[run][0][:, :n_holdout_true], data_holdout[run][1][:, :n_holdout_true], data_holdout[run][22][:, :n_holdout_true], data_holdout[run][22][:, :n_holdout_true],
                            data_test[0], data_test[1], data_test[2], data_test[8],
                            # numiter = numiter, batchsize = batchsize, long_factor = long_factor,
                            #   loss_stop = loss_stop, tol = tol, stop = stop,
                            gammas = gammas, lambdas = lambdas, lambda_max = None, lambda_min_ratio = None, num_lambda = None,
    ...
    regret_all_IPW_tree_DR.append(regret_temp)
    validation_all_IPW_tree_DR.append(validation_temp)
    time_all_IPW_tree_DR.append(time_temp)
    loss_all_IPW_tree_DR.append(loss_temp)
    
    pd.concat(regret_all_IPW_tree_DR).to_csv("regret_no_kernel_tree_CDR.csv", index = False)
    pd.concat(validation_all_IPW_tree_DR).to_csv("validation_no_kernel_tree_CDR.csv", index = False)
    pd.concat(time_all_IPW_tree_DR).to_csv("time_no_kernel_tree_CDR.csv", index = False)
    pickle.dump(loss_all_IPW_tree_DR, open("loss_no_kernel_tree_CDR.pkl", "wb"))
