...
def Double_robust(X_train_False,X_train, Y_transpose_Z_observed_train, Y_transpose_Z_expected_train,Z_train,
    ...
    Sigma_matrix_adjustment_ideal_lambda_1=Sigma_matrix+np.eye(d,d)
    Sigma_matrix_adjustment_sample_lambda_1_inverse=np.linalg.inv(Sigma_matrix_adjustment_sample_lambda_1)
    Sigma_matrix_adjustment_ideal_lambda_1_inverse=np.linalg.inv(Sigma_matrix_adjustment_ideal_lambda_1)
    ...
    for i in range(m):     
        ...
        Vanilla_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_inverse,Z_2[:,i])*c
        Clip_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Modeified_Ideal_Sigma_inverse,Z_2[:,i])*c
        Lambda_Perfect_Double_robust_predict_ideal_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_adjustment_ideal_lambda_1_inverse,Z_2[:,i])*c
        #Vanilla_Double_robust_predict_with_sample_Sigma[:,i]= Y_2_edge_expected[:,i]+np.dot(Sigma_matrix_sample_inverse,Z_2[:,i])*c
        Linear_IPW_ideal[:,i]=np.dot(Sigma_matrix_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_sample[:,i]=np.dot(Sigma_matrix_sample_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_ideal_Clip[:,i]=np.dot(Modeified_Ideal_Sigma_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_sample_Clip[:,i]=np.dot(Modeified_sample_Sigma_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_ideal_Lambda[:,i]=np.dot(Sigma_matrix_adjustment_ideal_lambda_1_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        Linear_IPW_sample_Lambda[:,i]=np.dot(Sigma_matrix_adjustment_sample_lambda_1_inverse,Z_2[:,i])*np.array(Y_2.iloc[i,:])
        
    return(model_estimate_2,Vanilla_Double_robust_predict_2_sample,Clip_Double_robust_predict_2_sample,Vanilla_Perfect_Double_robust_predict_ideal_Sigma,Clip_Perfect_Double_robust_predict_ideal_Sigma,
           Lambda_Double_robust_predict_2_sample,Lambda_Perfect_Double_robust_predict_ideal_Sigma,Linear_IPW_ideal,Linear_IPW_sample,Linear_IPW_ideal_Clip,
           Linear_IPW_sample_Clip,Linear_IPW_ideal_Lambda,Linear_IPW_sample_Lambda)
...
for i in range(len(n_train_seq)):
    ...
    with open(output, 'a') as f:
        print("Linear IPW Sample")
        print("Linear IPW Sample", file = f)
    ...
    regret_all_Linear_IPW_Sample.append(regret_temp)
    validation_all_Linear_IPW_Sample.append(validation_temp)
    time_all_Linear_IPW_Sample.append(time_temp)
    loss_all_Linear_IPW_Sample.append(loss_temp)
    
    pd.concat(regret_all_Linear_IPW_Sample).to_csv("regret_no_kernel_Linear_IPW_Sample.csv", index = False)
    pd.concat(validation_all_Linear_IPW_Sample).to_csv("validation_no_kernel_Linear_IPW_Sample.csv", index = False)
    pd.concat(time_all_Linear_IPW_Sample).to_csv("time_no_kernel_Linear_IPW_Sample.csv", index = False)
    pickle.dump(loss_all_Linear_IPW_Sample, open("loss_no_kernel_Linear_IPW_Sample.pkl", "wb"))
