...
for i in range(len(n_train_seq)):
    ...
    for k in range(runs):
        ...
        (c,d)=data_train_stratified[k][m][1].shape
        temp_False_model_parameter=np.zeros((70,a))
        temp_True_model_parameter=np.zeros((70,c))
        for m in range(70):
              (a,b)=data_train_stratified[k][m][0].shape
              if b !=0:
                  ...
                  False_model_parameter=False_model.coef_
                  temp_False_model_parameter[m,:]=False_model_parameter
                  True_model.fit(np.transpose(X_true), np.transpose(Y))
                  True_model_parameter=True_model.coef_
                  temp_True_model_parameter[m,:]=True_model_parameter                
                  ...
                  temp_False_model_parameter[m,:]=False_model_parameter
                  temp_True_model_parameter[m,:]=True_model_parameter
        True_parameter_matrix_list[k].append(temp_True_model_parameter)
        False_parameter_matrix_list[k].append(temp_False_model_parameter)
